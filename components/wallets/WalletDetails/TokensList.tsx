...
import { Card, CardBody, Pagination, Button } from "@heroui/react";
...
export const TokensList: React.FC<TokensListProps> = ({
  ...
  const processPositions = useCallback(
    (positions: WalletPosition[]) => {
      ...
      const totalValue = positions.reduce(
        (sum, pos) => sum + (pos.attributes?.value || pos?.value || 0),
        0,
      );
      ...
      return positions.map((position, index) => ({
        ...
        quantity:
          position.attributes?.quantity?.numeric ||
          position.attributes?.quantity_float ||
          position?.quantity ||
          0,
        ...
        isVerified:
          position.attributes?.verified || position?.is_verified || false,
        tokenAddress:
        ...
        icon:
          ...
          position.attributes?.icon ||
          position?.icon_url ||
          "",
        percentage:
          totalValue > 0
            ? ((position.attributes?.value || position?.value || 0) /
                totalValue) *
              100
            : 0,
      }));
  ...
  const filteredAndSortedTokens = useMemo(() => {
    let filtered = processedTokens.filter((token) => {
      ...
      if (hideDustTokens && token.value > 0 && token.value <= DUST_THRESHOLD)
        return false;
  ...
  const portfolioMetrics = useMemo(() => {
    ...
    const totalGainers = validTokens.filter(
      (token) => token.change24h > 0,
    ).length;
    const totalLosers = validTokens.filter(
      (token) => token.change24h < 0,
    ).length;
          ...
          <div className="fixed inset-0 bg-blur   backdrop-blur-sm z-50 flex items-center justify-center">
            <Card className="border border-divider shadow-lg">
              <CardBody className="flex items-center gap-1 p-6">
                ...
                <span className="text-sm font-medium mt-6 text-default-600">
                  Loading tokens...
                </span>
        ...
        <CardBody className="py-20">
          <div className="text-center max-w-sm mx-auto">
            ...
            <p className="text-default-500 mb-6">
              This wallet doesn't have any token positions, or they haven't been
              synced yet.
            </p>
            {onRefresh && (
              <Button
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-sm text-default-500 flex items-center gap-2">
            ...
            <span>
              ...
              {Math.min(endIndex, filteredAndSortedTokens.length)} of{" "}
          ...
          <Pagination
            ...
            classNames={{
              ...
              cursor:
                "bg-gradient-to-b shadow-lg from-primary-500 to-primary-600 dark:from-primary-600 dark:to-primary-700 text-white font-semibold rounded-lg",
            }}
            color="primary"
            page={currentPage}
            size="sm"
    ...
    <div className="flex items-center justify-center gap-3">
      ...
      {(filterMode !== "all" || searchQuery) && (
        ...
        >
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-secondary/20 to-primary/20 opacity-0 hover:opacity-100 transition-opacity" />
        </Button>
      )}
    </div>
  </div>
