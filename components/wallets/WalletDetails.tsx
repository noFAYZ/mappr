...
import type { WalletData } from "@/lib/wallet-analytics/types";
      ...
      <div className="h-64 flex items-center justify-center bg-default-50 dark:bg-default-900 rounded-lg">
        <div className="text-center">
          ...
          <p className="text-xs text-default-400 mt-1">
            Sync wallet to view performance
          </p>
        </div>
      </div>
    );
...
const TransactionHistory = ({
  ...
  const getTransactionIcon = useCallback((type: string) => {
    switch (type) {
      case "send":
        return <Send className="w-4 h-4 text-danger-500" />;
      case "receive":
        return <Download className="w-4 h-4 text-success-500" />;
      case "swap":
        return <Repeat className="w-4 h-4 text-warning-500" />;
      case "approve":
        return <CheckCircle2 className="w-4 h-4 text-primary-500" />;
      default:
        return <Activity className="w-4 h-4 text-default-500" />;
    }
  }, []);
...
const WalletDetails: React.FC<WalletDetailsProps> = ({
  ...
  const positions = useMemo(() => {
    ...
    if (
      data.lastSyncData?.positions &&
      Array.isArray(data.lastSyncData.positions)
    )
      return data.lastSyncData.positions;

    console.warn(
      "WalletDetails: Unable to extract positions from data structure:",
      data,
    );
    ...
    <div className="space-y-6 h-full">
      ...
      >
        ...
        {selectedTab === "analytics" && (
          <AnalyticsTab data={data} showBalances={showBalances} />
        )}
      </ModernTabs>
    </div>
  );
