...
import { HugeiconsBlockchain06, SolarWalletBoldDuotone } from "../icons/icons";
...
const MetricDisplay: React.FC<{
  ...
  showBalances?: boolean;
  index: number;
}> = ({ icon: Icon, label, value, trend, showBalances = true, index }) => {
  return (
    <motion.div
      animate={{ opacity: 1, y: 0 }}
      className="group"
        ...
        <div className="lg:col-span-3">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-0 divide-x divide-default-200  space-y-1">
            {metrics.map((metric, index) => (
              <MetricDisplay key={metric.label} {...metric} index={index} />
            ))}
          </div>
        </div>
      </div>
