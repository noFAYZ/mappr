...
import { Button, Progress, Card } from '@heroui/react';
...
const ConversationStats = memo(({ messages }: { messages: Message[] }) => {
  const stats = useMemo(() => {
    ...
    const totalWords = messages.reduce((acc, m) => acc + (m.metadata?.wordCount || m.content.split(/\s+/).length), 0);
    const avgReadTime = messages.reduce((acc, m) => acc + (m.metadata?.estimatedReadTime || 1), 0);
    ...
    <Card className="p-3 mb-4 bg-default-50 dark:bg-default-900/20">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
        <div className="text-center">
          <div className="font-semibold text-lg text-primary-600">{stats.totalMessages}</div>
          <div className="text-default-500">Messages</div>
        </div>
        <div className="text-center">
          <div className="font-semibold text-lg text-success-600">{stats.totalWords}</div>
          <div className="text-default-500">Words</div>
        </div>
        <div className="text-center">
          <div className="font-semibold text-lg text-warning-600">{stats.avgReadTime}m</div>
          <div className="text-default-500">Read Time</div>
        </div>
        <div className="text-center">
          <div className="flex justify-center gap-1">
            {stats.hasCode && <span className="px-1 py-0.5 bg-blue-100 text-blue-600 rounded text-xs">Code</span>}
            {stats.hasFinancial && <span className="px-1 py-0.5 bg-green-100 text-green-600 rounded text-xs">Finance</span>}
          </div>
...
export const MessageContainer: React.FC<MessageContainerProps> = memo(({
  ...
  const updateVisibleRange = useCallback(() => {
    ...
    const end = Math.min(displayMessages.length, Math.ceil((scrollTop + containerHeight) / itemHeight) + 5);
  ...
  return (
    <div className={`flex flex-col h-full ${className || ''}`}>
      ...
      <div 
        ...
        style={{ 
          height: enableVirtualization ? displayMessages.length * 150 : 'auto'
        }}
      >
