...
export const ContentRenderer: React.FC<ContentRendererProps> = React.memo(
  ({ content, metadata, isError = false }) => {
    ...
    useEffect(() => {
      ...
      const parseContent = async () => {
        try {
          ...
          const financialData =
            ContentParserService.extractFinancialData(content);
    ...
    const readingTime = Math.max(
      1,
      Math.ceil(parsedContent.metadata.wordCount / 250),
    );
    ...
    return (
      <ErrorBoundary>
        <div className="space-y-3">
          ...
          {/* Recommendation Card */}
          {metadata?.type === "recommendation" && <RecommendationCard />}
        </div>
      </ErrorBoundary>
    );
  },
    ...
    <div className="space-y-1.5">
      {insights.slice(0, 3).map((insight, index) => (
        <div key={index} className="flex items-start gap-2">
          ...
          <span className="text-blue-700 dark:text-blue-300 text-xs leading-relaxed">
            {insight}
          </span>
        </div>
      ))}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
      {financialData.currencies.length > 0 && (
        <div className="text-emerald-700 dark:text-emerald-300">
          ...
          {financialData.currencies.slice(0, 3).join(", ")}
        </div>
      )}
      {financialData.percentages.length > 0 && (
        <div className="text-emerald-700 dark:text-emerald-300">
          <span className="font-medium">Changes:</span>{" "}
          {financialData.percentages.slice(0, 3).join(", ")}
        </div>
      )}
    </div>
  </Card>
