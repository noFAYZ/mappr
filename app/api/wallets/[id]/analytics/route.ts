...
export async function GET(
  ...
  try {
    ...
    const { data: snapshots, error: snapshotsError } = await supabase
      ...
      .select('total_value, day_change, day_change_percent, snapshot_date, positions_count, chains_count')
      .eq('wallet_id', id)
    ...
    const nftPortfolio = latestSyncData?.nftPortfolio || { items: [], totalCount: 0 };
    ...
    const positionAllocation = positions
      .map((pos: any) => ({
        ...
        percentage: totalPositionValue > 0 ? ((pos.attributes?.value || 0) / totalPositionValue) * 100 : 0,
    ...
    const chainDistribution = positions.reduce((acc: Record<string, any>, pos: any) => {
    ...
    const chainData = Object.values(chainDistribution)
      .map((chain: any) => ({
        ...chain,
        percentage: totalPositionValue > 0 ? (chain.value / totalPositionValue) * 100 : 0
      }))
    ...
    const transactionSummary = {
      ...
      sent: transactions.filter((tx: any) => tx.attributes?.direction === 'sent').length,
      received: transactions.filter((tx: any) => tx.attributes?.direction === 'received').length,
      failed: transactions.filter((tx: any) => tx.attributes?.status === 'failed').length,
      totalVolume: transactions.reduce((sum: number, tx: any) => 
        sum + (tx.attributes?.value || 0), 0
      ),
      totalFees: transactions.reduce((sum: number, tx: any) => 
        sum + (tx.attributes?.fee || 0), 0
      )
    };
    ...
    const insights = {
      ...
      worstPerformer: positionAllocation
      ...
      diversification: {
        ...
        herfindahlIndex: positionAllocation.reduce((sum, pos) => 
          sum + Math.pow(pos.percentage / 100, 2), 0
    ...
    previousPeriodEnd.setDate(previousPeriodEnd.getDate() - getTimeframeDays(timeframe));
    const previousPeriodStart = new Date(previousPeriodEnd);
    previousPeriodStart.setDate(previousPeriodStart.getDate() - getTimeframeDays(timeframe));
    ...
    const previousValue = previousSnapshots?.[0]?.total_value ? Number(previousSnapshots[0].total_value) : null;
    const periodComparison = previousValue ? {
      previousValue,
      currentValue: analytics.currentValue,
      change: analytics.currentValue - previousValue,
      changePercent: ((analytics.currentValue - previousValue) / previousValue) * 100
    } : null;
      ...
      data: {
        ...
        nfts: {
          ...
          collections: nftPortfolio.items ? 
            [...new Set(nftPortfolio.items.map((nft: any) => nft.attributes?.collection).filter(Boolean))].length : 0
        },
        transactions: transactionSummary,
        insights,
        comparison: periodComparison,
        timeframe,
        lastUpdated: latestSyncData?.metadata?.lastSyncAt || wallet.last_sync_at
      }
    });
    ...
    switch (action) {
      case 'create_snapshot':
        ...
        const { data: snapshotResult, error: snapshotError } = await supabase
          .rpc('create_portfolio_snapshot', { wallet_uuid: id });
