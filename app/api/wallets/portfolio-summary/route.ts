...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalValue =
      summary?.reduce(
        (sum, wallet) => sum + (Number(wallet.total_value) || 0),
        0,
      ) || 0;
    const totalChange =
      summary?.reduce(
        (sum, wallet) => sum + (Number(wallet.day_change) || 0),
        0,
      ) || 0;
    const totalPositions =
      summary?.reduce(
        (sum, wallet) => sum + (wallet.positions_count || 0),
        0,
      ) || 0;
    const uniqueChains = new Set(summary?.map((wallet) => wallet.chain_type))
      .size;
    ...
    return NextResponse.json({
      data: {
        ...
        performanceDistribution: {
          excellent:
            summary?.filter((w) => w.performance_rating === "excellent")
              .length || 0,
          good:
            summary?.filter((w) => w.performance_rating === "good").length || 0,
          neutral:
            summary?.filter((w) => w.performance_rating === "neutral").length ||
            0,
          poor:
            summary?.filter((w) => w.performance_rating === "poor").length || 0,
        },
      },
    });
  } catch (error: any) {
