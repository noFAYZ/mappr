...
export async function POST(request: NextRequest) {
  try {
    ...
    const { walletIds, syncType = 'full_sync', options = {} } = await request.json();
    ...
    if (!wallets || wallets.length === 0) {
      return NextResponse.json({ error: 'No valid wallets found' }, { status: 404 });
    }
    ...
    return NextResponse.json({ 
      ...
      estimatedCompletionTime: new Date(Date.now() + wallets.length * 30000).toISOString()
    });
