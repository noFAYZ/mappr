...
function selectBestHFModel(message: string, context: any): HuggingFaceModel {
  ...
  if (
    ...
    messageLower.includes("breakdown")
  ) {
...
function createFinancialSystemPrompt(
  context: any,
...
${context.portfolios
  .map((portfolio) => `• ${portfolio.name}: ${portfolio.itemCount} holdings`)
  .join("\n")}
`
...
async function callHuggingFaceServerless(
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 503) {
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, 5000 * (attempts + 1)),
          );
          attempts++;
          continue;
        }
      ...
      if (
        ...
        assistantMessage.length > 10
      ) {
        console.log(
          `✅ Success with ${model.name} (${assistantMessage.length} chars)`,
        );
  ...
  throw new Error(
    `Failed to get response from ${model.name} after ${maxAttempts} attempts`,
  );
}
...
async function callWithHFFallback(
  ...
  for (const model of models) {
    try {
      const response = await callHuggingFaceServerless(
        message,
        context,
        messageHistory,
        model,
      ...
      return NextResponse.json(
        {
          ...
          message:
            "Hugging Face token not configured. Please add HF_TOKEN to your environment variables.",
        },
        { status: 500 },
      );
    }
    ...
    return {
      ...
      portfolios:
        portfolios?.map((p) => ({
          ...
          itemTypes: [
            ...new Set(p.portfolio_items?.map((item) => item.item_type) || []),
          ],
        })) || [],
      dataActivity: {
