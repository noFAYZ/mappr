...
export class ContentParserService {
  ...
  private static readonly PATTERNS = {
    ...
    currency:
      /(?:\$|USD|EUR|GBP|JPY|CAD|AUD|CHF|CNY)[\s]?[\d,]+(?:\.?\d*)?(?:[BMK])?/gi,
    percentage: /[+-]?\d+(?:\.\d+)?%/g,
    financialNumber:
      /\b\d{1,3}(?:,\d{3})*(?:\.\d+)?(?:\s?(?:[BMK]|billion|million|thousand))?/gi,
  ...
  private static getCalloutColor(icon: string): string {
    ...
    return (
      colorMap[icon] ||
      "bg-default-50 dark:bg-default-950/20 border-default-500"
    );
  }
  ...
  private static processInlineFormatting(text: string): string {
    ...
    processed = processed.replace(
      ...
      '<code class="px-1 py-0.5 bg-default-100 dark:bg-default-800 rounded text-xs font-mono">$1</code>',
    );

    // Process inline math
    processed = processed.replace(
      this.PATTERNS.mathInline,
      '<span class="px-1 py-0.5 bg-primary-100 dark:bg-primary-900 rounded font-mono text-primary-800 dark:text-primary-200">$1</span>',
    );
    ...
    processed = processed.replace(
      ...
      '<del class="line-through text-default-500">$1</del>',
    );
    ...
    // Process auto-links
    processed = processed.replace(
      this.PATTERNS.autoLinks,
      '<a href="$1" class="text-primary-600 dark:text-primary-400 hover:underline" target="_blank" rel="noopener noreferrer">$1</a>',
    );
  ...
  private static highlightFinancialData(content: string): string {
    ...
    content = content.replace(this.PATTERNS.percentage, (match) => {
      const isPositive =
        match.startsWith("+") ||
        (!match.startsWith("-") && parseFloat(match) > 0);
  ...
  static extractInsights(content: string): string[] {
    ...
    sentences.forEach((sentence) => {
      ...
      if (
        insightPatterns.some((pattern) => pattern.test(trimmed)) &&
        insights.length < 5
      ) {
  ...
  static extractFinancialData(content: string): ParsedContent["financialData"] {
    const currencies = [...content.matchAll(this.PATTERNS.currency)].map(
      (m) => m[0],
    );
    const percentages = [...content.matchAll(this.PATTERNS.percentage)].map(
      (m) => m[0],
    );
    const numbers = [...content.matchAll(this.PATTERNS.financialNumber)].map(
      (m) => m[0],
    );
  ...
  private static detectFinancialContent(content: string): boolean {
    const hasFinancialPatterns =
      this.PATTERNS.currency.test(content) ||
      this.PATTERNS.percentage.test(content);

    const hasFinancialKeywords = Array.from(this.FINANCIAL_KEYWORDS).some(
  ...
  private static analyzeContentSemantic(content: string): {
    sentiment: "positive" | "neutral" | "negative";
    topics: string[];
  } {
    const positiveWords = [
  ...
  private static calculateReadingLevel(
    wordCount: number,
  ...
  private static cleanupCache(): void {
    if (this.cacheMap.size > CHAT_MESSAGE_CONFIG.CACHE_SIZE) {
      ...
      const toDelete = entries.slice(
        0,
        entries.length - CHAT_MESSAGE_CONFIG.CACHE_SIZE,
      );
