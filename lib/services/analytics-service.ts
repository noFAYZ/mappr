...
export class AnalyticsService {
    static async calculatePortfolioMetrics(
      ...
      try {
        ...
        const totalReturn = Number(latest.total_value) - Number(earliest.total_value);
        const totalReturnPercent = Number(earliest.total_value) > 0 
        ...
        const annualizedReturn = this.annualizeReturn(totalReturnPercent, timeframe);
        const annualizedVolatility = this.annualizeVolatility(volatility / avgValue, timeframe);
    ...
    private static annualizeReturn(returnPercent: number, timeframe: string): number {
      const days = {
        '1d': 1,
        '7d': 7,
        '30d': 30,
    ...
    private static annualizeVolatility(volatility: number, timeframe: string): number {
      const days = {
        '1d': 1,
        '7d': 7,
        '30d': 30,
