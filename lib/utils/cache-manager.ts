export class CacheManager {
  ...
  constructor(
    namespace: string,
    config: { defaultTTL: number; maxSize: number },
  ) {
  ...
  private cleanup(): void {
    ...
    if (this.cache.size >= this.config.maxSize) {
      ...
      const itemsToRemove =
        this.cache.size - Math.floor(this.config.maxSize * 0.8);
  ...
  getStats() {
    const namespaceItems = Array.from(this.cache.entries()).filter(([key]) =>
    ...
    return {
      ...
      oldestItem: namespaceItems.reduce(
        (oldest, [, item]) =>
