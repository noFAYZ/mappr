...
const formatTime = (
    ...
    if (timestamp < 978307200000) { // Jan 1, 2001 in milliseconds
      ...
      if (debug) console.log('formatTime: Converted seconds to milliseconds:', timestamp, '->', normalizedTimestamp);
    }
  
    // Validate the normalized timestamp creates a valid date
    const testDate = new Date(normalizedTimestamp);
    if (isNaN(testDate.getTime())) {
      if (debug) console.warn('formatTime: Invalid date created from timestamp:', normalizedTimestamp);
      return 'Invalid date';
    }
    ...
    if (dateYear < 1970 || dateYear > currentYear + 10) {
      if (debug) console.warn('formatTime: Date outside reasonable range:', dateYear);
      return 'Invalid date';
    }
      ...
      switch (period) {
        ...
        case 'week': {
          ...
          const isThisWeek = (
            ...
            date < new Date(today.getFullYear(), today.getMonth(), today.getDate() - today.getDay() + 7)
          );
  ...
  const formatBusinessTime = (
    ...
    const { showBusinessHours = false, showMarketStatus = false, timezone = 'America/New_York', debug = false } = options || {};
    ...
    if (isNaN(date.getTime())) {
      if (debug) console.warn('formatBusinessTime: Invalid date from timestamp:', timestamp);
      return 'Invalid date';
    }
    ...
    if (showBusinessHours) {
      ...
      const businessStatus = isBusinessHours ? '🟢 Business Hours' : '🔴 After Hours';
  ...
  try {
    ...
    if (timestamp instanceof Date) {
      ...
      if (/^\d+(\.\d+)?$/.test(timestamp.trim())) {
        ...
        date = new Date(numericValue < 9999999999 ? numericValue * 1000 : numericValue);
      } else {
        // Date string
        date = new Date(timestamp);
      }
      if (debug) console.log('toTimestampz: String input parsed to:', date.toISOString());
      ...
      if (timestamp.toString().length <= 10) {
        ...
        if (debug) console.log('toTimestampz: High precision timestamp converted');
      }
    } else {
...
const toTimestampzPresets = {
  ...
  est: (timestamp: number | string | Date, includeMicroseconds = false) => 
    toTimestampz(timestamp, { timezone: 'America/New_York', includeMicroseconds }),
  
  pst: (timestamp: number | string | Date, includeMicroseconds = false) => 
    toTimestampz(timestamp, { timezone: 'America/Los_Angeles', includeMicroseconds }),
  ...
  withOffset: (timestamp: number | string | Date, offset: string, includeMicroseconds = false) => 
...
const isValidTimestampz = (timestampz: string): boolean => {
  const timestamptzRegex = /^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}(\.\d{3,6})?([+-]\d{2}):?(\d{2})?$/;
 ...
 */
const parseTimestampz = (timestampz: string): {
  date: string;
  time: string;
  milliseconds?: string;
  microseconds?: string;
  ...
  const regex = /^(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2}:\d{2})(\.\d{3}(\d{3})?)?([+-])(\d{2}):?(\d{2})?$/;
  const match = timestampz.trim().match(regex);
  
  if (!match) return null;
  
  const [, date, time, fullMs, microseconds, sign, hours, minutes = '00'] = match;
  ...
  try {
    ...
    if (targetTimezone && targetTimezone !== 'original') {
      try {
        ...
        displayDate = new Date(utcDate.toLocaleString('en-US', { timeZone: targetTimezone }));
        displayTimezone = targetTimezone;
        if (debug) console.log('timestampzToReadable: Converted to timezone:', targetTimezone);
      } catch (e) {
        if (debug) console.warn('timestampzToReadable: Timezone conversion failed, using original');
      }
    }
    ...
    switch (format) {
      ...
      case 'custom':
        ...
        return timestampzToReadable(timestampz, { ...options, format: 'smart' });
 ...
 */
const parseTimestampzString = (timestampz: string, debug: boolean = false): {
  date: Date;
  timezone: string;
  components: {
    year: number;
  ...
  const regex = /^(\d{4})-(\d{2})-(\d{2})[T\s](\d{2}):(\d{2}):(\d{2})(\.\d{3,6})?([+-])(\d{2}):?(\d{2})?$/;
  const match = trimmed.match(regex);
  
  if (!match) {
    if (debug) console.warn('parseTimestampzString: Failed to match regex:', trimmed);
    ...
    for (const fallbackRegex of fallbackFormats) {
      ...
      if (fallbackMatch) {
        if (debug) console.log('parseTimestampzString: Matched fallback format');
        // Handle Z timezone or missing timezone
        const [, year, month, day, hour, minute, second, fractional] = fallbackMatch;
        const isZulu = trimmed.endsWith('Z');
        const adjustedMatch = [
          trimmed, year, month, day, hour, minute, second, fractional || '',
          isZulu ? '+' : '+', '00', '00'
        ];
...
const parseMatchedTimestamp = (match: string[] | RegExpMatchArray, debug: boolean = false): {
  date: Date;
  timezone: string;
  components: {
    year: number;
  ...
  const [, year, month, day, hour, minute, second, fractional, tzSign, tzHours, tzMinutes = '00'] = match;
  ...
  if (isNaN(date.getTime())) {
    if (debug) console.warn('parseMatchedTimestamp: Invalid date created:', isoString);
 ...
 */
const formatRelativeTime = (date: Date, locale: string, debug: boolean = false): string => {
  const now = new Date();
  ...
  try {
    ...
    if (debug) console.warn('formatRelativeTime: Intl.RelativeTimeFormat failed, using fallback');
  }
...
const formatBusinessReadable = (
  ...
  if (dateOnly.getTime() === today.getTime()) {
    ...
    const diffDays = Math.abs(Math.floor((dateOnly.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)));
    if (diffDays <= 7) {
...
const timestampzPresets = {
  // Quick formats
  quick: (timestampz: string) => timestampzToReadable(timestampz, { format: 'short' }),
  full: (timestampz: string) => timestampzToReadable(timestampz, { format: 'full' }),
  relative: (timestampz: string) => timestampzToReadable(timestampz, { format: 'relative' }),
  business: (timestampz: string) => timestampzToReadable(timestampz, { format: 'business' }),
  
  // Timezone specific
  local: (timestampz: string, timezone: string) => 
    timestampzToReadable(timestampz, { targetTimezone: timezone, showTimezone: true }),
  
  utc: (timestampz: string) => 
    timestampzToReadable(timestampz, { targetTimezone: 'UTC', showTimezone: true }),
    
  est: (timestampz: string) => 
    timestampzToReadable(timestampz, { targetTimezone: 'America/New_York', showTimezone: true }),
    
  pst: (timestampz: string) => 
    timestampzToReadable(timestampz, { targetTimezone: 'America/Los_Angeles', showTimezone: true }),
    
  ist: (timestampz: string) => 
    timestampzToReadable(timestampz, { targetTimezone: 'Asia/Kolkata', showTimezone: true }),
...
const examples = () => {
  ...
  const recentTimestampz = recentTimestamp.toISOString().replace('T', ' ').replace('Z', '+00');
...
export const timestampzToChart = (
  ...
  try {
    ...
    const isoString = timestampz
      .replace(' ', 'T')
      .replace(/([+-]\d{2}):?(\d{2})?$/, (match, hours, mins = '00') => `${hours}:${mins}`);
