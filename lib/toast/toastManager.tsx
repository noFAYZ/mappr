    ...
    completed: (walletName: string, totalValue?: number, change?: number) => {
      return this.addToast({
        ...
        metadata: {
          amount: totalValue,
          percentage:
            change && totalValue ? (change / totalValue) * 100 : undefined,
        },
        actions: [
          {
            ...
            handler: () => {
              if (navigator.share) {
                ...
                navigator.clipboard.writeText(
                  `My wallet ${walletName} portfolio value: ${totalValue ? new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(totalValue) : "N/A"}`,
                );
              }
            },
            variant: "ghost" as const,
          ...
          },
          {
            ...
            handler: () => {
              if (navigator.share) {
                ...
                navigator.clipboard.writeText(
                  `I just reached ${milestone} in my crypto portfolio!`,
                );
        ...
        actions: [
          {
            label: "Try It Now",
            handler: () =>
              (window.location.href = actionUrl || "/premium/features"),
            variant: "primary" as const,
        ...
        actions: [
          {
            label: "View Results",
            handler: () =>
              (window.location.href = resultsUrl || "/ai/analysis"),
            variant: "primary" as const,
            icon: <ExternalLink className="w-3 h-3" />,
          },
        ],
