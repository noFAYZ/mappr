...
export const useEnhancedWalletAnalytics = (
  config: Partial<EnhancedWalletAnalyticsConfig> = {},
) => {
  ...
  const syncWalletEnhanced = useCallback(
        ...
        if (result) {
          ...
          if (finalConfig.enableMilestoneNotifications && currentValue > 0) {
            ...
            const newMilestone = finalConfig.milestoneThresholds.find(
              (threshold) =>
                threshold > lastMilestone && currentValue >= threshold,
            );
          ...
          if (
            finalConfig.enableRiskAlerts &&
            Math.abs(changePercent) >= finalConfig.riskThreshold
          ) {
            setTimeout(() => {
              const riskLevel =
  ...
  const generateAIInsight = useCallback(
    (wallet: UserWallet, changePercent: number, currentValue: number) => {
      const insights = [
        {
      ...
      if (applicableInsight && finalConfig.enableAIInsights) {
        toastManager.onAIInsight(
          applicableInsight.message,
          applicableInsight.confidence,
        );
      }
    },
  ...
  // Enhanced add wallet
  const addWalletEnhanced = useCallback(
    async (
      address: string,
      name?: string,
      ...
      try {
        const result = await walletAnalytics.addWallet(
          address,
          name,
          chainType,
        );
        ...
        if (result) {
          ...
          if (finalConfig.enableSyncNotifications) {
            setTimeout(() => {
              const newWallet = walletAnalytics.wallets.find(
                (w) => w.address === address,
              );
        ...
        toastManager.showError(
          ...
          error.message ||
            "Could not add wallet. Please check the address and try again.",
        );
  ...
  const removeWalletEnhanced = useCallback(
  ...
  const syncAllWalletsEnhanced = useCallback(
    async (options: SyncOptions = {}): Promise<void> => {
      ...
      const progressHandler = toastManager.showProgressToast(
        "Syncing All Wallets",
        0,
      );
      let completed = 0;
      ...
      try {
        const syncPromises = walletAnalytics.wallets.map(
          async (wallet, index) => {
            try {
  ...
  useEffect(() => {
    if (
      !finalConfig.enablePerformanceAlerts &&
      !finalConfig.enableTransactionNotifications
    )
      return;
    ...
    const monitorPerformance = () => {
      walletAnalytics.wallets.forEach((wallet) => {
        ...
        if (data?.portfolio) {
          const previousValue =
            previousPortfolioRef.current.get(wallet.id) || 0;
          ...
          if (previousValue > 0) {
            ...
            if (Math.abs(changePercent) >= finalConfig.performanceThreshold) {
              ...
              const lastAlertTime =
                lastPerformanceAlertRef.current.get(wallet.id) || 0;
              const now = Date.now();
        ...
        if (data) {
          ...
          const recentTransactions =
            data.transactions?.filter(
              (tx) =>
                Date.now() - new Date(tx.timestamp).getTime() <
  ...
  return {
    ...
    showCustomNotification: (type: string, message: string, metadata?: any) => {
      switch (type) {
        case "achievement":
          return toastManager.onAchievement(
            message,
            metadata?.rarity || "common",
          );
        case "security":
          return toastManager.onSecurityAlert(
            message,
    ...
    triggerTestNotifications: () => {
      ...
      setTimeout(() => {
        toastManager.onAIInsight(
          "Your portfolio shows strong diversification across DeFi protocols",
          92,
        );
      }, 1000);
    ...
    // Batch operations
    batchAddWallets: async (
      wallets: Array<{ address: string; name?: string; chainType?: string }>,
    ) => {
      const results = [];

      for (const wallet of wallets) {
        const result = await addWalletEnhanced(
          wallet.address,
          wallet.name,
          wallet.chainType,
        );
